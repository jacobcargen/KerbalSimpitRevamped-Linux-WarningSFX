...
# Copy voice warning audio files bundled with the plugin so GameDatabase can load them
# Now using new folder structure from distrib/voice_warnings/
if [ -d "distrib/voice_warnings" ]; then
    mkdir -p "$KSP_DIR/GameData/KerbalSimpit/voice_warnings"
    cp -r distrib/voice_warnings/* "$KSP_DIR/GameData/KerbalSimpit/voice_warnings/" 2>/dev/null || true
    echo "Copied voice_warnings (audio assets with new folder structure) to GameData/KerbalSimpit/voice_warnings/"
    echo "  Structure: each warning has beep.ogg and voice/ subfolder with voice clips"
fi
